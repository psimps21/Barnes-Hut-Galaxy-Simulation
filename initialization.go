...
func InitializeGalaxy0(numOfStars int, r, x, y float64) Galaxy {
	...
	for i := range g {
		...
		speed := 0.75 * math.Sqrt(G*blackHoleMass/dist) // approximation of orbital velocity equation: half of true speed to prevent instability
...
// InitializeGalaxy takes number of stars in the galaxy, radius of the galaxy to be constructed,
// and center of galaxy to be constructed. Returns a spinning Galaxy object -- which is just a slice of Star pointers
func InitializeGalaxy1(numOfStars int, r, x, y float64) Galaxy {
	g := make(Galaxy, numOfStars)
